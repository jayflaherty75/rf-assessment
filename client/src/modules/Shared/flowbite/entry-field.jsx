...
const EntryField = ({ description, buttonText, name, value, inputRef, onChange, onSubmit }) => (
    <form>
        <label htmlFor="search" className="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-gray-300">{description}</label>
        <div className="relative">
            <input type="search" id={`${name}Input`} name={name} ref={inputRef} value={value} onChange={onChange} className="block p-4 pl-10 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder={description} required></input>
